...
CREATE POLICY "Patients can view their own progress"
    ...
    USING (
        EXISTS (
            ...
            WHERE patients.id = patient_education_progress.patient_id
...
CREATE POLICY "Providers can view patient progress"
    ON patient_education_progress FOR SELECT
    TO authenticated
    USING (
        EXISTS (
...
CREATE POLICY "Anyone can view education content tags"
    ON education_content_tags FOR SELECT
    TO authenticated
    USING (true);

CREATE POLICY "Only providers can manage education content tags"
    ON education_content_tags FOR ALL
    TO authenticated
    USING (
        EXISTS (
            SELECT 1 FROM providers
