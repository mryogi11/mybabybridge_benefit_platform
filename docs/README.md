...
*   **Schema Definition:** The database schema is defined in TypeScript at `src/lib/db/schema.ts`. This includes tables like `users`, `providers`, `appointments`, `benefit_packages`, etc., and columns like `theme_preference` on the `users` table for storing user theme settings.
